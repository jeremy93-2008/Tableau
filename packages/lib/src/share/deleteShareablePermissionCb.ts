...
export async function deleteShareablePermissionCb(options: {
    ...
    // if is the case we return false, owner is always the owner, and cannot be deleted
    if (
    ...
    // 3. A user can decide to leave a board, and so can delete himself
    // independently of his role. So if the User line to delete is the same
    // that the user session we leave him to delete himself
    if (boardUserSharingToEdit.user.email === session.user.email) return true

    // 4. Administrator cannot delete another Administrator, and so we need to
    ...
    // 5. We check if the user have the administrator permission, that mean
    // having canEditSchema && canEditContent == true, if it's the case
    // we allow the deletion
    return (
